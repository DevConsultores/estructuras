...
{{ $exclude_urls := (getenv "VARNISH_EXCLUDE_URLS" "^(/update\\.php|/([a-z]{2}/)?admin|/([a-z]{2}/)?admin/.*|/([a-z]{2}/)?system/files/.*|/([a-z]{2}/)?flag/.*|.*/ajax/.*|.*/ahah/.*|.*/revive/.*)$") }}
...
sub vcl_recv {
  ...
  if (req.http.User-Agent ~ "iP(hone|od)" || req.http.User-Agent ~ "Android" || req.http.User-Agent ~ "Symbian" || req.http.User-Agent ~ "^BlackBerry" || req.http.User-Agent ~ "^SonyEricsson"
      ...
      || req.http.User-Agent ~ "mobile" || req.http.User-Agent ~ "(?i)Mozilla.*Android.*Mobile*|iPhone|BlackBerry|Opera Mobi|Opera Mini|SymbianOS|UP.Browser|J-PHONE|Profile/MIDP|portalmmm|DoCoMo|Obigo|Galaxy Nexus|GT-I9300|GT-N7100|HTC One|Nexus [4|7|S]|Xoom|XT907")
  {
    set req.http.X-Device = "mobile";
    set req.http.X-Mobile = "1";
  }
  if (req.http.User-Agent ~ "tablet" || req.http.User-Agent ~ "iPad" || req.http.User-Agent ~ "android" && req.http.User-Agent !~ "mobile") {
...
sub vcl_hash {
    ...
    #Si no es un elemento estatico le meteremos al hash tmb más cosas
    if (!(req.url ~ "\.(gif|jpg|jpeg|swf|swd|flv|mp3|mp4|pdf|ico|png|gz|tgz|bz2|css|zip|htm|html|xml|txt|js)(\?.*)?$")) {
         #Le metemos el Device para saber si es PC/Movil/Tablet
         hash_data(req.http.X-Device); 

          #Le metemos el País
          #Quitado 14 ago, no tiene caso pq es lo mismo para todos los países, esto despúes se puede quitar para mostrar
          #Listados diferentes y contenidos recomandos diferentes según el país
          #if(req.http.X-Country) { hash_data(req.http.X-Country); }

          #Aquí esto es para hashear la cookie de sesión de drupal quitando el resto
          #31ago17 yon: (regresado) | Cancelado pq no queremos cachear por cookie serían mcuhas versiones
         # if (req.http.Cookie) {
